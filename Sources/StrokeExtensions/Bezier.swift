...
internal class Bezier {
  ...
  class func bezier_point_x(t: CGFloat, a: CGPoint, b: CGPoint, c: CGPoint, d: CGPoint) -> CGFloat {
  ...
  class func bezier_point_y(t: CGFloat, a: CGPoint, b: CGPoint, c: CGPoint, d: CGPoint) -> CGFloat {
  ...
  class func bezier_length(start: CGPoint, p1: CGPoint,  p2: CGPoint, finish: CGPoint, accuracy: UInt) -> CGFloat {
    ...
    for item in stride(from: 0, to: 1, by: by) {
      dot.x = Bezier.bezier_point (t: item, start: start.x, control_1: p1.x, control_2: p2.x, finish: finish.x)
      dot.y = Bezier.bezier_point (t: item, start: start.y, control_1: p1.y, control_2: p2.y, finish: finish.y)
      if (item > 0) {
  ...
  class func bezier_arcLengths(start: CGPoint, p1: CGPoint, p2: CGPoint, finish: CGPoint, accuracy: UInt) -> [CGFloat] {
    
    var clen:CGFloat = 0
    
    let by: CGFloat = 1.0 / CGFloat(accuracy)
    
    var ox:CGFloat = Bezier.bezier_point_x (t: 0, a: start, b: p1, c: p2, d: finish);
  ...
  class func bezier_evenlyDistributed(u: CGFloat, arcLengths: [CGFloat]) -> CGFloat {
